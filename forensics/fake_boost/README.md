...
Pcap file
By extracting HTTP object I get a script :
```
...
```
Explain : `$jozeq3n.ToCharArray()` -> big variable turn to array stored in `$s0yAY2gmHVNFd7QZ`
`[array]::Reverse($s0yAY2gmHVNFd7QZ)` -> big variable reversed
` -join $s0yAY2gmHVNFd7QZ 2>&1> $null` -> PROUT !

`$LOaDcODEoPX3ZoUgP2T6cvl3KEK = [sYSTeM.TeXt.ENcODING]::UTf8.geTSTRiNG([SYSTEm.cOnVeRT]::FRoMBaSe64sTRing("$s0yAY2gmHVNFd7QZ")) ;` -> base64 -d
...
`rj1893rj1joijdkajwda` found in HTTP object, it base64 then aes encrypt
icyberchef recipe : 
From base64 --> AES Decrypt (key from base64 == Y1dwaHJOVGs5d2dXWjkzdDE5amF5cW5sYUR1SWVGS2k=, IV == 0000000000000000000000000000000 (not the good one), CBC, from RAW to RAW)
Give :
```
[
    {
...
```
When base64 decode first email, we get : *b3W4r3_0f_T00_g00d_2_b3_7ru3_0ff3r5}* looks like the garbage at the begin should be HTB{ dont know why not correctly decrypt
Put the IV at the beginning of the encrypted file : `[byte[]] $fullData = $aesManaged.IV + $encryptedData`
To get the good IV : from base64 to hex, extract the 32 first hex char (6c41beac6711c9829eaa5cd76f441555) after deleting the hex IV just go encode in base64 the rest of the data finally decode with correct IV and key
The `part1` variable contain the beginning of the flag
